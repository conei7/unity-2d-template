...
UIはすべてUnity上のSceneまたはPrefabとして通常配置しています。
実行時にUIを自動生成する実装は使用していません。
...
1. `Extras`シーンから対象機能のボタンを削除します。
2. 対象の機能シーンをBuild Settingsから外します。
3. 対象シーンとCatalogを削除します。
4. 他から参照されていなければ、対応するControllerスクリプトを削除します。
